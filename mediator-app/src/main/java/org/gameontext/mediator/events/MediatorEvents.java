...
public class MediatorEvents {
	...
	@Inject KafkaRxJavaObservable kafka;
	
	public EventSubscription subscribeToPlayerEvents(String userId, PlayerEventHandler peh){
        Subscription subscription = kafka.consume()
                .filter(event -> "playerEvents".equals(event.getTopic()))
                .filter(event -> userId.equals(event.getKey()))
                .subscribe(event -> handlePlayerEvent(event, peh));
